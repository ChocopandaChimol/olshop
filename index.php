...
if (isset($_POST['add_to_cart'])) {
    ...
    $select_cart = mysqli_query($conn, "SELECT * FROM `cart` WHERE name = '$product_name' AND user_id = '$user_id'") or die('Query gagal!');

    if (mysqli_num_rows($select_cart) > 0) {
        $message[] = 'Produk sudah ditambahkan ke keranjang';
    } else {
        mysqli_query($conn, "INSERT INTO `cart` (user_id, name, price, image, quantity) VALUES ('$user_id', '$product_name', '$product_price', '$product_image', '$product_quantity')") or die('Query gagal');
...
if (isset($_POST['update_cart'])) {
    ...
    mysqli_query($conn, "UPDATE `cart` SET quantity = '$update_quantity' WHERE id = '$update_id'") or die('Query gagal!');
    $message[] = 'Update jumlah keranjang berhasil!';
}

if (isset($_GET['remove'])) {
    $remove_id = $_GET['remove'];
    mysqli_query($conn, "DELETE FROM `cart` WHERE id = '$remove_id'") or die('Query gagal!');
...
<head>
    ...
    <style>
        /* Menambahkan CSS untuk mengubah warna teks ID pengguna */
        .user-profile span {
            color: #00b894; /* Menggunakan warna hijau cerah untuk ID pengguna */
        }
    </style>
</head>
<body>
...
<div class="container">
    ...
    <div class="products">
        ...
        <div class="box-container">
            <?php 
            $select_product = mysqli_query($conn, "SELECT * FROM `products`") or die('Query gagal!');
